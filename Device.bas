...
Sub Globals
	...
	Dim PHPURL As String = $"https://192.168.8.117/Company/controller/device.php?action=get_device&id=${Branch.branch_selected}"$
...
Sub LoadCompanyData
	PHPURL = $"https://192.168.8.117/Company/controller/device.php?action=get_device&id=${Branch.branch_selected}&page=${pageNo}&limit=${limit}"$
	Log(PHPURL)
	Dim Job1 As HttpJob
	Job1.Initialize("", Me)
